/*
  ...
  v 0.1a
    Fixed mov ax,  instruction.

  v 0.1b
    Fixed memory movement bug.

  v 0.1c
    Added dup statement.  Simple Math

  v 0.1d  3/28/96
    Fixed a few problems with the table and multiple data words.
    Improved on the Simple Math

  v 0.1e  4/4/96
    Added Equates and lea instruction.

          4/11/96
    commented out DEBUG, added send in bugs statement

  v 0.1f  4/12/96
    4/12/96 allowed for [],immed to be used
    4/21/96 compressed the executable
    4/22/96 added the ability to use variables

  v 0.11f 4/23/96
    4/22/96 added the ability to use variables

  v 0.11g 5/26/96
    Fixed variable forward referencing

*/

#define HEADER "General 8086 Assembler 0.11g Copyright 1996 Jim Gage.\n"\
               " Please report any bugs to jrg@engr.uark.edu."
...
#define filename
/* at the beginning of each line there could be one of three things,
   an opcode, a comment, or a label */

/* these could be shortened to int */
...
#define pointer 0x00000005 /* should be replaced by 0x00000008 later */
...
#define READBUFFERSIZE  512     /* allow up to 256 bytes due to equates */
...
/* table to store label names and values */
...
tregisters registers[] =
{
  ...
  "sp" ,0x04,regsp|reg16 ,"bp" ,0x05,reg16|regbp ,"si" ,0x06,reg16|regsi,
...
enum eopcodes { opnone,opaaa,opaad,opaam,opaas,opadc,opadd,opand,opclc,
                opcld,opcli,opcall,opcbw,opcmc, opcmp, opcmpsb,opcmpsw,
                opcwd,opdaa,opdas,opdec,opdiv,ophlt,opidiv,opimul,opin,
                opinc,opint,opiret,opja,opjae,opjb,opjbe,opjc,opjcxz,
                opje,opjg,opjge,opjl,opjle,opjmp,opjmpf,opjna,opjnae,opjnb,
                opjnbe,opjnc,opjne,opjng,opjnge,opjnl,opjnle,opjno,opjnp,
                opjns,opjnz,opjo,opjp,opjpe,opjpo,opjs,opjz,oplahf,oplds,
                oplea,oples,oplodsb,oplodsw,oploop,oploope,oploopne,oploopnz,
                oploopz,opmov,opmovsb,opmovsw,opmul,opneg,opnop,opnot,opor,
                opout,oppopf,oppushf,oprcl,oprcr,oppop, oppush, oprep,
                oprepe,oprepne,oprepnz,oprepz,opret,opretf,oprol,opror,
                opsahf,opsal,opsar,opsbb,opsub,opscasb,opscasw,opshl,
                opshr,opstc,opstd,opsti,opstosb,opstosw,optest,opwait,
                opxchg,opxlat,opxor,
                op32bit
              };
